...
func (db *depositsDB) UpdateDepositsStatus(requestId string, blockNumber uint64) error {
	result := db.gorm.Table("deposits"+requestId).Where("status = ? and block_number <= ?", 0, blockNumber).Updates(map[string]interface{}{"status": 1})
	if result.Error != nil {
...
func (db *depositsDB) StoreDeposits(requestId string, depositList []Deposits, depositLength uint64) error {
	result := db.gorm.Table("deposits"+requestId).CreateInBatches(&depositList, int(depositLength))
	if result.Error != nil {
